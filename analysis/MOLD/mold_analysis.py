...
- Per frame, compute end-to-end distance as the Euclidean distance between the
  ...
  (D-band length proxy).
- Identify crosslink marker sets from the bond network (LZS/LZD pairs (MOLD)) and compute
...
Helper (duo markers via m1/m2):
  python mold_analysis.py \
  --top reduced.tpr \
  --traj traj_vmd.xtc \
  --sel1 "resname ACE" --sel2 "resname NME" \
  --m1-resname LZS --m2-resname LZD \
...
# NEW DEFAULT MARKERS (duo)
LZS_RESNAME_DEFAULT = "LZS"
LZD_RESNAME_DEFAULT = "LZD"
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            ...
            "Crosslink = duo LZS+LZD identificato via bonds."
        )
    )
    ...
    # Updated marker defaults
    p.add_argument("--m1-resname", default=LZS_RESNAME_DEFAULT, help="Resname marker 1 (default: LZS)")
    p.add_argument("--m2-resname", default=LZD_RESNAME_DEFAULT, help="Resname marker 2 (default: LZD)")
...
def detect_duos(u, m1_resname="LZS", m2_resname="LZD"):
    """
...
def run_one_traj(top, traj, sel1, sel2, m1_resname, m2_resname, start, stop, stride, dt_ps_fallback):
    ...
    if len(duos) == 0:
        raise ValueError("No valid duo found.")
    if bad > 0:
    ...
    for ts in u.trajectory[i0:i1:st]:
        ...
        for i in range(n_duos):
            pd = coords_nm[duo_idx[i]].mean(axis=0)
