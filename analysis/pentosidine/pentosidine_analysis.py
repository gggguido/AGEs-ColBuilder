...
- Per frame, compute end-to-end distance as the Euclidean distance between the
  ...
  (D-band length proxy).
- Identify crosslink marker sets from the bond network (APD/LPS pairs (pentosidine)) and compute
...
Helper (duo markers via m1/m2):
  python pentosidine_analysis.py \
  --top reduced.tpr \
  --traj traj_vmd.xtc \
  --sel1 "resname ACE" --sel2 "resname NME" \
  --m1-resname APD --m2-resname LPS \
...
# Default markers (duo) used for clustering
APD_RESNAME_DEFAULT = "APD"
LPS_RESNAME_DEFAULT = "LPS"
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            ...
            "Crosslink = duo APD+LPS identified via bonds."
        )
    )
    ...
    # Marker defaults updated to APD/LPS
    p.add_argument("--m1-resname", default=APD_RESNAME_DEFAULT, help="Marker 1 resname (default: APD)")
    p.add_argument("--m2-resname", default=LPS_RESNAME_DEFAULT, help="Marker 2 resname (default: LPS)")
...
def detect_duos(u, m1_resname="APD", m2_resname="LPS"):
    """
...
def run_one_traj(top, traj, sel1, sel2, m1_resname, m2_resname, start, stop, stride, dt_ps_fallback):
    ...
    n_duos = len(duos)
    if n_duos % 2 != 0:
        raise ValueError(f"Odd number of duos: {n_duos}. For k=2 balanced split, it must be even.")
    ...
    for ts in u.trajectory[i0:i1:st]:
        ...
        p1 = c1.mean(axis=0)  # mean position of ACE
        p2 = c2.mean(axis=0)  # mean position of NME
        ...
        for i in range(n_duos):
            pd = coords_nm[duo_idx[i]].mean(axis=0)  # mean position of duo
